...
public sealed class ZadaniaLinq
{
    ...
    {
        var emails = DaneUczelni.Studenci.Select(s => $"{s.Email}");
        return emails;
    }
    ...
    {
        var sortedStudents = DaneUczelni.Studenci.OrderBy(s => s.Nazwisko)
                                                                        .ThenBy(s => s.Imie)
    ...
    {
        if (DaneUczelni.Zapisy.Any(s => !s.CzyAktywny))
        {
            return new[] { "True" };
        }
        else
    ...
    {
        return new []{DaneUczelni.Prowadzacy.All(p => !string.IsNullOrWhiteSpace(p.Katedra))?"TAK":"NIE"};
    }
    ...
    {
        var activeCases = DaneUczelni.Zapisy.Count(s => s.CzyAktywny == true);
        return new[] {activeCases.ToString()};
    }
    ...
    {
        var intersectCities = DaneUczelni.Studenci.Select(s => s.Miasto).Distinct();
        return intersectCities;
    }
    ...
    {
        var threeCases = DaneUczelni.Zapisy.Select(s => $"{s.DataZapisu}, {s.StudentId}, {s.PrzedmiotId}")
                                                            .Take(3);
        return threeCases;
    }
    ...
    {
        var secondPage = DaneUczelni.Przedmioty.OrderBy(p => p.Nazwa)
            .Skip(2).Take(2).Select(przedmiot => $"{przedmiot.Nazwa}, {przedmiot.Kategoria}");
        return secondPage;
    }
    ...
    public IEnumerable<string> Zadanie11_PolaczStudentowIZapisy()
    {
        return DaneUczelni.Studenci.Join(DaneUczelni.Zapisy, student => student.Id,
                ...
                (student, zapis) => 
                    $"{student.Imie} {student.Nazwisko}, {zapis.DataZapisu}");
    }
    ...
    {
        var Joined = DaneUczelni.Zapisy.Join(DaneUczelni.Przedmioty,
            ...
            temp => temp.zapis.StudentId,
            student => student.Id,
            (arg1, student) => $"{student.Imie} {student.Nazwisko}, {arg1.przedmiot.Nazwa}");
        return Joined;
    }
    ...
    {
        var studia = DaneUczelni.Przedmioty.Join(DaneUczelni.Zapisy,
                                                przedmiot => przedmiot.Id,
                                                zapis => zapis.PrzedmiotId,
                                                (przedmiot, zapis) => new
                                                {
                                                    przedmiot = przedmiot.Nazwa
                                                }).GroupBy(przedmiot => przedmiot.przedmiot)
    ...
    {
        return DaneUczelni.Przedmioty.Join(DaneUczelni.Zapisy,
                przedmiot => przedmiot.Id,
                zapis => zapis.PrzedmiotId,
                (przedmiot, zapis) => new { przedmiot.Nazwa, zapis.OcenaKoncowa })
            .Where(temp => temp.OcenaKoncowa != null)
            .GroupBy(p => p.Nazwa)
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        return DaneUczelni.Prowadzacy.GroupJoin(DaneUczelni.Przedmioty,
            ...
            (prowadzacy, przedmioty) => new
            {
                ...
                przedmiots = przedmioty.Count()
            }).Select(temp => $"{temp.imie} {temp.nazwisko}: {temp.przedmiots}");
    }
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        return DaneUczelni.Studenci.Join(DaneUczelni.Zapisy,
            ...
            (student, zapis) => new
            {
                ...
                mark = zapis.OcenaKoncowa
            }).Where(temp => temp.mark!=null)
            .GroupBy(temp => new {temp.name, temp.surname})
            .Select(temp => $"{temp.Key.name} {temp.Key.surname} - {temp.Max(x => x.mark)}");
    }
    ...
    public IEnumerable<string> Wyzwanie01_StudenciZWiecejNizJednymAktywnymPrzedmiotem()
    {
        return DaneUczelni.Studenci.Join(DaneUczelni.Zapisy,
                ...
                ((student, zapis) => new
                {
                    name = student.Imie,
                    surname = student.Nazwisko,
                    available = zapis.CzyAktywny
                })).Where(temp => temp.available)
            .GroupBy(temp => new { temp.name, temp.surname })
            .Where(temp => temp.Count() > 1)
            .Select(temp => $"{temp.Key.name} {temp.Key.surname}: {temp.Count()}");
    }
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        return DaneUczelni.Prowadzacy
            ...
            .Join(DaneUczelni.Zapisy,
                temp => temp.p.Id,
                z => z.PrzedmiotId,
                (temp, z) => new { temp.pr.Imie, temp.pr.Nazwisko, z.OcenaKoncowa })
            .Where(x => x.OcenaKoncowa != null)
            .GroupBy(x => new { x.Imie, x.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko}: {g.Average(x => x.OcenaKoncowa):F2}");
    }
